...
define('scalejs.tabs-jqueryui/tabsBindings', {
    ...
    'tabs-more-item': function (ctx) {
        return {
            style: {
                color: ctx.$parent.active() === ctx.$index() ? 'coral' : 'white'
            },
            text: this.header
        };
    }
    ...
    function wrapValueAccessor(valueAccessor, element) {
        return function () {
            ...
            function calculateWidth() {
                ...
                tabsContainerWidth = $tabs.offset().left + tabsContainerWidth > window.innerWidth ?
                    window.innerWidth - $tabs.offset().left
                    : tabsContainerWidth;
            ...
            function createAddTab() {
                ...
                if (data.defaultItems().length === 0) return;
                ...
                $tab.unbind().click(function (e) {
                    ...
                    } else if (data.defaultItems().length === 1) {
                        var newTab = unwrap(data.defaultItems)[0];
                        data.active(data.itemsSource().length);
                        if (newTab.create) {
                            data.itemsSource.push(newTab.create());
                        } else {
                            data.itemsSource.push(newTab);
                        }
                    }
                });
            }
            ...
            if (data.defaultItems) {
                data.menuItems = ko.computed({
                    read: function () {
                        var newItems = toEnumerable(unwrap(data.defaultItems)).select(function (item) {
                            var menuItem = merge(item, {
                                addTab: function () {
                                    $menu.add.bPopup({ opacity: 0 }).close();
                                    data.active(data.itemsSource().length);
                                    data.itemsSource.push(item.create ? item.create() : item);
                                }
                            });
                            return menuItem;
                        }).toArray()
            ...
            data.active = data.active || ko.observable(0);
            ...
            data.openMore = function () {
                ...
                $menu.more.bPopup({
                    follow: [false, false],
                    position: [$tab.offset().left + 10, $tab.offset().top + $tab.innerHeight() -3],
                    opacity: 0,
                    speed: 0
                });
            }
